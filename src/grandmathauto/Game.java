...
/**
 * Contains game and applet logic
 */
public class Game implements Runnable {
	...
	// Selecting options
	private STATE[] mainArr = { STATE.SKILL_LEVEL, STATE.OPTIONS, STATE.SCORES, STATE.CREDITS, STATE.QUIT };
	...
	public void run() {
		/**
		 * Main game loop, switches based on the game state to handle both menu
		 * logic and game logic
		 */
		while (running) {
			switch (State) {
			/* In main menu */
			...
			case GAME:
				...
				if (firstRun) {
					...
					player = new Car(3 * Main.windowWidth / 8, Main.windowHeight - 70);
	...
	private void handleObstacles() {
		...
		case 1: // 8 random cones
			if (obstacleSchemeTracker % (int) (timeBetweenObstacles / 8) == 0) {
				int xPosition = rand
						.nextInt((Background.roadBarrierRight - ConeObstacle.width) - Background.roadBarrierLeft)
		...
		case 2: // 3 horizontal line of cones
			if (obstacleSchemeTracker % (int) (timeBetweenObstacles / 3) == 0) {
				int xPosition = rand
						.nextInt((Background.roadBarrierRight - (ConeObstacle.width * 4)) - Background.roadBarrierLeft)
						+ Background.roadBarrierLeft;
				ConeObstacle cone = new ConeObstacle(xPosition, -ConeObstacle.height);
		...
		case 3: // center opening
			if (obstacleSchemeTracker == (int) (timeBetweenObstacles / 2)) {
				...
				int xPosition = Background.roadBarrierLeft
						+ (int) ((Background.roadBarrierRight - Background.roadBarrierLeft) / 2)
						- (2 * ConeObstacle.width);
				ConeObstacle cone = new ConeObstacle(xPosition, -ConeObstacle.height);
				ConeObstacle cone1 = new ConeObstacle(xPosition + ConeObstacle.width, -ConeObstacle.height);
				ConeObstacle cone2 = new ConeObstacle(xPosition + (2 * ConeObstacle.width), -ConeObstacle.height);
				ConeObstacle cone3 = new ConeObstacle(xPosition + (3 * ConeObstacle.width), -ConeObstacle.height);
