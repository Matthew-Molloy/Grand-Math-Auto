...
/**
 * Handles the painting of graphical components of the applet including sprites
 * and text.
 */
public class GraphicsManager {
	static public Image image, currentSprite, playerImage, backgroundImage, debugDot, leftArrow, rightArrow, checkMark,
			checkBox, coneImage, connect;

	private Main main;

	private String[] menuOptions = { "Grand Math Auto", "Play", "Options", "High Scores", "Credits", "Quit" };
	private String[] optOptions = { "Sound Effects", "Background Music", "Back" };
	private String[] levelOptions = { "Select Skill Level", "Addition", "Subtraction", "Multiplication", "Start Game" };
	...
	public GraphicsManager(Main main) throws ClassNotFoundException {
		...
		backgroundImage = main.getImage(main.getBase(), "data/road_background.png");
	...
	/**
	 * Primary function for redrawing graphical elements. Depending on the state
	 * of the game, paints a different screen.
	 */
	public void paint(Graphics g) {
		...
		case MAIN:
			...
			case 0:
				g.drawImage(rightArrow, x - fm.stringWidth(menuOptions[1]), 167, main);
				g.drawImage(leftArrow, x + (fm.stringWidth(menuOptions[1]) / 2) + 10, 167, main);
				break;

			case 1:
				g.drawImage(rightArrow, x - (fm.stringWidth(menuOptions[2]) / 2) - 50, 227, main);
				g.drawImage(leftArrow, x + (fm.stringWidth(menuOptions[2]) / 2) + 10, 227, main);
				break;

			case 2:
				g.drawImage(rightArrow, x - (fm.stringWidth(menuOptions[3]) / 2) - 50, 287, main);
				g.drawImage(leftArrow, x + (fm.stringWidth(menuOptions[3]) / 2) + 10, 287, main);
				break;

			case 3:
				g.drawImage(rightArrow, x - (fm.stringWidth(menuOptions[4]) / 2) - 50, 347, main);
				g.drawImage(leftArrow, x + (fm.stringWidth(menuOptions[4]) / 2) + 10, 347, main);
				break;

			case 4:
				g.drawImage(rightArrow, x - fm.stringWidth(menuOptions[5]), 407, main);
				g.drawImage(leftArrow, x + (fm.stringWidth(menuOptions[5]) / 2) + 10, 407, main);
				break;
			}
			break;
		...
		case GAME:
			if (!game.getFirstRun()) {
				g.drawImage(backgroundImage, game.getBg1().getBgX(), game.getBg1().getBgY(), main);
				g.drawImage(backgroundImage, game.getBg2().getBgX(), game.getBg2().getBgY(), main);
				g.drawImage(currentSprite, game.getPlayer().getPositionX(), game.getPlayer().getPositionY(), main);
